...
//rotate the device until point to the north
...
do{
    ...
    if((mid_angle > my_heading)&(my_heading > max_angle)){                 // turn right
            move_time= ((my_heading - max_angle)*(5.459) + 50);
            move_right();
            Move_for_time();
            }
    if((mid_angle <= my_heading)&(my_heading < min_angle)){                 // turn left
...
void GPS_Mode_Move(){
  ...
  if ((lon_Target - longitude) >= 1){        // the diffrirenc more than 15m in direction of East
  point_to(East_min,East_max);
  move_time=(lon_Target - longitude)*5400u;
  move_forward();
  Move_for_time();
  }
  if ((longitude - lon_Target) >= 1){        // the diffrirenc more than 15m in direction of West
  point_to(West_min,West_max);
  move_time=(longitude - lon_Target)*5400u;
  move_forward();
  Move_for_time();
  }
  if ((lat_Target - latitude) >= 1){        // the diffrirenc more than 15m in direction of North
  point_to(North_min,North_max);
  move_time=(lat_Target - latitude)*5400u;
  move_forward();
  Move_for_time();
  }
  if ((latitude - lat_Target) >= 1){        // the diffrirenc more than 15m in direction of South
